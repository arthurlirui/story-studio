#!/usr/bin/env bash
# 封面 v3：书名用古典楷书(玄冬楷书)，作者=独孤元景，生成768x1024再缩放600x800
set -u
cd /data/openclaw/workspace/story-studio
FONTS="$PWD/fonts"
TFONT="$FONTS/kai_xuandong.ttf"
OUT="series/重生穿越/covers_v3"
mkdir -p "$OUT"
...
gen () {
  ...
  timeout 500 python3 tools/book_cover_comfy.py \
    ...
    --title-font "$TFONT" --author-font "$TFONT" 2>&1 | tail -1
}
...
gen "三国神医" "华佗之上，我在乱世救苍生" 616 \
"Book cover, premium novel cover artwork, cinematic Chinese historical fiction, Three Kingdoms era, a military field tent interior with a wooden medicine chest and silver acupuncture needles, distant battlefield with fire and warships on a river at night seen through the tent opening, healer silhouette, tense wartime healing mood, centered composition, $NEG, dramatic cinematic lighting, high detail, painterly realistic illustration, professional publishing cover design, deep crimson, ink green, dark bronze and warm lamplight color palette"
...
gen "靖康不耻" "经济学教授穿越北宋，富国更强国" 919 \
"Book cover, premium novel cover artwork, cinematic Chinese historical fiction, late Northern Song dynasty, prosperous Bianjing city skyline partly in flames at dusk, Jin dynasty armored cavalry pressing at the distant city gate, tension of an empire on the brink of collapse, centered composition, $NEG, dramatic cinematic lighting, high detail, painterly realistic illustration, professional publishing cover design, ash gray, ink blue, dark red and cold bronze color palette"
...
for f in sorted(glob.glob(os.path.join(outdir, "*_cover.png"))):
    ...
    img = Image.open(f).convert("RGB").resize((600,800), Image.LANCZOS)
