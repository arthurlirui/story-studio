...
只输出英文Midjourney prompt。"""
    ...
    story, cover = await asyncio.gather(
        call_llm(client, [{"role":"system","content":"你是历史共情写作者，文风冷峻克制有温度。"},{"role":"user","content":story_prompt}], api_base=API_BASE, api_key=API_KEY, model=MODEL, max_tokens=3000),
        call_llm(client, [{"role":"system","content":"You are an AI art prompt expert."},{"role":"user","content":cover_prompt}], api_base=API_BASE, api_key=API_KEY, model=MODEL, max_tokens=800)
    )
